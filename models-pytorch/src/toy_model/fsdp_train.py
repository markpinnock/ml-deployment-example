...
def main(rank: int, world_size: int) -> None:
    ...
    my_auto_wrap_policy = functools.partial(
        size_based_auto_wrap_policy, min_num_params=2
    )
    ...
    if torch.cuda.is_available():
        ...
        fsdp_model = FSDP(
            model, auto_wrap_policy=my_auto_wrap_policy, cpu_offload=False
        )
    else:
    ...
    # Use barrier to make sure training is done on all ranks
    dist.barrier()
    states = fsdp_model.state_dict()
    if rank == 0:
    ...
    optimiser = torch.optim.Adam(fsdp_model.parameters(), lr=learning_rate)
    ...
    for epoch in range(epochs):
        ...
        train_loss = train(
            fsdp_model, data_dict["train_loader"], loss_obj, optimiser, rank
        )
        valid_loss = test(fsdp_model, data_dict["valid_loader"], loss_obj, rank)
    ...
    if rank == 0:
        ...
        with torch.inference_mode():
            ...
            pred_train_ys = fsdp_model(train_data).squeeze()
            pred_valid_ys = fsdp_model(valid_data).squeeze()
